...
from ofdft_normflows.flax_ode import FullODENet
...
def training(t_kin: str = 'TF',
    ...
    test_point_wscore = next(gen_batches)
    test_point_wscore = test_point_wscore[:1]
    test_point = test_point_wscore[:, :-3]
    # -----------------------------------

    # define flows
    model_fwd = FullODENet(3, nn_arch, True, True)  # data to noise
    model_rev = FullODENet(3, nn_arch, False, False)  # noise to data

    params = model_fwd.init(key, test_point_wscore)
    _ = model_rev.init(key, test_point)
    # -----------------------------------
