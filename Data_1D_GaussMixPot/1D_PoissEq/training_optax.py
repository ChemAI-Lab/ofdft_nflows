...
def train(act, n_neurons, layers,
          epochs: int = 2500, batch_size: int = 512,
          bool_load_params: bool = True):
    rng = jrnd.PRNGKey(0)
    ...
    def batches_generator(key: prng.PRNGKeyArray, batch_size: int):
        while True:
            ...
            y = -4*jnp.pi*normal_dist(x)

            _, key = jrnd.split(key)
            x0_bc = jrnd.uniform(key, shape=(
                int(batch_size/2), 1), minval=x_min, maxval=-2.)
            _, key = jrnd.split(key)
            x1_bc = jrnd.uniform(key, shape=(
                int(batch_size/2), 1), minval=2., maxval=x_max)
...
def main():
    ...
    parser.add_argument("--n_neurons",    type=int,
                        default=512,      help="number of neurons per layer")
