...
@jax.jit
def f_h_den(x): return jnp.exp(-2 *
...
def plot_dft_distribution(FIG_DIR: str):
    ...
    m = DFTDistribution(atoms=atoms, geometry=coords, spin=1)
    ...
    # plt.clf()
    contour1 = ax1.contourf(xx, zz, rho_exact_2D.reshape(xx.shape),
                            linestyles='dashed', levels=25, vmin=0, vmax=Ne)
...
def training(t_kin: str = 'TF',
    ...
    for i in range(epochs+1):
        ...
        if i % 10 == 0 or i <= 25:
            ...
            ax.plot(xt, rho_exact,
                    color='k', ls=":", label=r"$\hat{\rho}(x)$" % norm_dft)
            ax.plot(xt, Ne*rho_pred,
