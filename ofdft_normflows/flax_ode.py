    ...
    def __call__(self, states, params):
        ...
        def f_ode(params, state, t):
            ...
            dx_and_dlopz, _f_vjp = jax.vjp(
                lambda state: _f_ode(params, state, t0*t), state)
...
class FullODENet(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        def init_fn(rng, x): return ode_func.init(
            random.split(rng)[-1], 0., x)['params']
        ode_func_params = self.param(
            'ode_func', init_fn, jnp.ones(self.in_out_dims))
        x_logpx_score_x = ODEBlockVmap(in_out_dims=self.in_out_dims,
                                       features=self.features,
                                       bool_neg=self.bool_neg, bool_wscore=self.bool_wscore,
                                       tol=self.tol)(inputs, ode_func_params)
        # return x
        x, logp_x, score_x = x_logpx_score_x[:, :self.in_out_dims], x_logpx_score_x[:,
                                                                                    self.in_out_dims:self.in_out_dims+1], x_logpx_score_x[:, -self.in_out_dims:]
        return x, logp_x, score_x
...
if __name__ == '__main__':
    ...
    prior_dist = MultivariateNormalDiag(
        jnp.zeros(dimension), 1.*jnp.ones(dimension))
