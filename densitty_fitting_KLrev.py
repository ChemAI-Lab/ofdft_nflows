...
def _plotting(rho_pred, rho_true, XY, _label: Any):
    ...
    ax.scatter(jnp.array([-0.76, 0.76])/2, jnp.zeros(
        2),  marker='o', color='k', s=35)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    plt.tight_layout()
    plt.savefig(f'Figures/H2_maxLL/CNF_rho_H2_{i}_H2_KLrev.png')
...
def main(batch_size, epochs):
    ...
    atoms = ['H', 'H']
    geom = jnp.array([[-0.76, 0., 0.], [0.76, 0., 0.]])/2
    ...
    for i in range(epochs+1):
        ...
        if i % 100 == 0:
            ...
            u0 = jnp.linspace(-2., 2., 12)
            u1 = jnp.linspace(-2.5, 2.5, 12)
...
def _main(batch_size, epochs):
    ...
    prior_dist = MultivariateNormalDiag(mean, cov)
    gen_batch = batch_generator(prior_dist, batch_size)
    z0_and_logp_z0 = next(gen_batch)
    print(prior_dist.log_prob(z0_and_logp_z0[:, :-1]))

    atoms = ['H', 'H']
    geom = jnp.array([[0., 0., 0.], [0.76, 0., 0.]])
    dft_dist = DFTDistribution(atoms, geom)
    dft_dist.prob(dft_dist, z0_and_logp_z0[:, :-1])
